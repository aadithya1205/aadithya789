...
with tab1:
    ...
    with st.form("my_form"):
        ...
        with col1:
            ...
            st.info("W:0, WI:1, S:2, Others:3, PL:4, IPL:5, SLAWR:6)
    ...
    if submit_button and flag == 0:
        x= np.array([[float(quantity_tons), application, (float(thickness)), float(width),
    ...
    #loading a trained model from pickle file
        pickled_model = pickle.load(open('regression.sav', 'rb'))
...
with tab2:
    ...
    if csubmit_button and cflag == 0:
        import pickle
        x = np.array([[float(cquantity_tons), float(cselling), capplication,
                                float(cthickness), float(cwidth), ccountry, int(ccustomer), int(product_ref),
                                citem_type]])
